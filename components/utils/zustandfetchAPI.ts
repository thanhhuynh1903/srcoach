...
interface ApiState {
  ...
  deleteNotification: (path: string, payload: any) => Promise<any>;
...
const useApiStore = create<ApiState>(set => ({
  ...
  deleteNotification: async (path: string, payload: any) => {
    set({isLoading: true, status: null});
  ...
  clear: () =>
    set({data: null, dataDetail: null, isLoading: false, status: null}),
}));
