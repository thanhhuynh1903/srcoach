...
import { View, Text, StyleSheet, TouchableOpacity, Modal, Alert, Animated, Easing, ScrollView } from "react-native"
    ...
    <View style={styles.card}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.daysContainer}>
        {dayNumbers.map((dayNum, idx) => {
          ...
          const hasMissed = currentDay?.ScheduleDetail?.some((workout) => workout.status === "MISSED")
          const isCompleted =
            currentDay?.ScheduleDetail?.length > 0 &&
            currentDay?.ScheduleDetail?.every((workout) => workout.status === "COMPLETED")
          return (
            <TouchableOpacity
              key={currentDay.id}
        ...
        {workouts.length > 0 && (
          <View style={styles.workoutDetailsContainer}>
            <Text style={styles.workoutDetailsTitle}>
              Workouts for {getMonthFromStartDate()} {selectedDay ? new Date(selectedDay.day).getDate() : ""}
            </Text>
