...
import { View, Text, StyleSheet, TouchableOpacity, Switch, Modal, Alert, Animated, Easing } from "react-native"
...
const EnhancedScheduleCard = ({
  ...
  const handleDeleteExpert = () => {
    Alert.alert("Delete Schedule", "Are you sure you want to delete this expert schedule?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete",
        style: "destructive",
        async onPress() {
          try {
            ...
            if (success) {
              ...
              Toast.show({
                type: "success",
                text1: "Expert schedule deleted successfully",
              });
            }
          } catch (error) {
            Toast.show({
              type: "error",
              text1: "Failed to delete expert schedule",
            });
          }
        },
      },
    ...
    <View style={styles.card}>
      ...
      <View style={styles.totalsContainer}>
        ...
        <View style={styles.totalItem}>
          ...
          <Text style={styles.totalValue}>{calculateTotals(daySchedules).totalSteps.toLocaleString()}</Text>
        ...
        <View style={styles.totalItem}>
          ...
          <Text style={styles.totalValue}>{calculateTotals(daySchedules).totalDistance.toFixed(1)} km</Text>
        ...
        <View style={styles.totalItem}>
          ...
          <Text style={styles.totalValue}>{calculateTotals(daySchedules).totalCalories.toLocaleString()}</Text>
      ...
      <View style={styles.daysContainer}>
        {days?.map((day) => {
          ...
          const isCompleted = currentDaySchedule?.workouts.every((workout) => workout.status === "COMPLETED")
            ...
            {selectedDaySchedule.workouts?.map((workout, index) => (
              <View key={index} style={styles.workoutItem}>
                <View style={styles.timeContainer}>
                  ...
                  >
                    <Text style={styles.workoutStatusText}>{workout.status}</Text>
                  </View>
                </View>
        ...
        <View style={styles.footerTags}>
          {isExpertChoice ? (
            <View style={styles.tagContainer}>
              <Icon name="ribbon-outline" size={14} color="#052658" style={styles.tagIcon} />
              <Text style={styles.tagText}>Expert's Choice</Text>
            </View>
          ) : (
            <View style={styles.tagContainer}>
              <Icon name="person-outline" size={14} color="#052658" style={styles.tagIcon} />
          ...
          {expertname && (
            <View style={styles.expertRole}>
              <Icon name="person-circle-outline" size={14} color="#052658" style={styles.tagIcon} />
              <Text style={styles.tagText}>{expertname}</Text>
            </View>
          )}
          {runnername && (
            <View style={styles.runnerRole}>
              <Icon name="person-circle-outline" size={14} color="#FFF" style={styles.tagIcon} />
              ...
              <>
                <View style={styles.modalAlarmSection}>
                  ...
                  <View style={styles.modalAlarmControl}>
                    <Text style={styles.modalAlarmText}>Enable workout reminders</Text>
                    <Switch
                      value={alarmEnabled}
                ...
                </View><View style={styles.modalDivider} />
              </>
            )}

            {(expert_id && isOwner) || !isExpertChoice ? (
              <>
                <TouchableOpacity style={styles.modalOption} onPress={handleEdit}>
                  <Icon name="create-outline" size={24} color={theme.colors.primaryDark} />
              ...
              <>
                <TouchableOpacity style={styles.modalOption} onPress={handleViewDetail}>
                  <Icon name="eye-outline" size={24} color={theme.colors.primaryDark} />
                  <Text style={styles.modalOptionText}>View Details</Text>
                </TouchableOpacity>

                <View style={styles.modalDivider} />

                <TouchableOpacity style={styles.modalOption} onPress={handleContact}>
                  <Icon name="arrow-redo-circle-outline" size={24} color={theme.colors.primaryDark} />
