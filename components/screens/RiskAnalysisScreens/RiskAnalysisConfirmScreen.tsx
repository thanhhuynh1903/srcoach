...
const RiskAnalysisConfirmScreen = () => {
  ...
  const {userActivity} = route.params as {userActivity: any};
  ...
  useEffect(() => {
    ...
    if (age && !isNaN(ageNum)) {
      setAgeError(ageNum < 13 ? 'Minimum age for running tracking is 13 years old' : 
                ageNum > 150 ? 'Maximum age allowed is 150 years' : null);
    } else if (age === '') setAgeError(null);

    const weightNum = parseFloat(weight);
    if (weight && !isNaN(weightNum)) {
      setWeightError(weightNum <= 20 ? 'Weight must be more than 20 kg' : 
                     weightNum > 500 ? 'Maximum weight allowed is 500 kg' : null);
    } else if (weight === '') setWeightError(null);

    const heightNum = parseFloat(height);
    if (height && !isNaN(heightNum)) {
      setHeightError(heightNum <= 20 ? 'Height must be more than 20 cm' : 
                    heightNum > 500 ? 'Maximum height allowed is 500 cm' : null);
    } else if (height === '') setHeightError(null);
  }, [age, weight, height]);

  const handleAnalyze = () => {
    if (limitData && limitData.limit <= 0) return;
    if (ageError) return showErrorDialog('Invalid Age', ageError);
    if (weightError) return showErrorDialog('Invalid Weight', weightError);
    if (heightError) return showErrorDialog('Invalid Height', heightError);
  ...
  const navigateToRiskWarning = () => {
    ...
    navigate.navigate('RiskWarningScreen', {params: {userActivity: updatedParams}});
  };
  ...
  const renderContentLoader = () => (
    <View style={styles.content}>
      <ContentLoader speed={1.5} width="100%" height="100%" viewBox="0 0 400 800" backgroundColor="#f3f3f3" foregroundColor="#ecebeb">
        ...
        {[...Array(8)].map((_, i) => <Rect key={i} x="0" y={110 + i * 70} rx="8" ry="8" width="100%" height="60" />)}
  ...
  const isButtonDisabled = () => !!ageError || !!weightError || !!heightError || (limitData?.limit === 0);

  const renderExerciseCard = (exercise: any) => (
    <TouchableOpacity
      key={exercise.id}
      style={[styles.exerciseCard, selectedExercise.id === exercise.id && styles.selectedCard]}
      onPress={() => setSelectedExercise(exercise)}>
      {exercise.isPopular && <Icon name="star" size={16} color="#4CAF50" style={styles.popularStar} />}
      <Icon name={exercise.icon} size={24} color={selectedExercise.id === exercise.id ? theme.colors.primary : '#64748B'} />
      <Text style={[styles.exerciseName, selectedExercise.id === exercise.id && styles.selectedText]}>
        {exercise.name}
      </Text>
    </TouchableOpacity>
  );
    ...
    <View style={styles.container}>
      ...
      {loading ? renderContentLoader() : (
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <Text style={styles.subtitle}>Please verify your information for accurate risk assessment</Text>
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.sectionDescription}>Select the type of running that you are/were planning to do</Text>

            <View style={styles.exerciseGrid}>
              {exerciseTypes.filter(ex => !ex.others).map(renderExerciseCard)}
            </View>
            <Text style={styles.sectionTitle}>Others</Text>
            <Text style={styles.sectionDescription}>Other running variants not included in typical real-life running</Text>
            <View>{exerciseTypes.filter(ex => ex.others).map(renderExerciseCard)}</View>
          </View>

          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Personal Information</Text>
            <Text style={styles.sectionDescription}>Update your details if needed</Text>
          ...
          {showAgeWarning && (
            <View style={styles.warningBox}>
              ...
              <Text style={styles.warningText}>
                You've changed your age from your account information. This analysis will now assess suitability for the specified age range that you've picked.
              </Text>
            </View>
          )}
          ...
          {showMeasurementWarning && (
            <View style={styles.warningBox}>
              <Icon name="alert-circle" size={16} color={theme.colors.warning} />
              <Text style={styles.warningText}>
                Without weight and height measurements, the analysis will provide general results rather than personalized recommendations.
              </Text>
            </View>
          )}
          ...
          <CommonDialog
            ...
            actionButtons={[{label: 'Got it', variant: 'contained', handler: () => setShowExerciseInfoDialog(false)}]}
          />
          ...
          <CommonDialog
            ...
            actionButtons={[
              {label: 'Cancel', variant: 'outlined', handler: () => setShowIncompleteDialog(false)},
              {label: 'Proceed', variant: 'contained', handler: navigateToRiskWarning}
            ]}
          />
        </ScrollView>
      )}
...
const styles = StyleSheet.create({
  ...
  header: {flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, gap: 8, backgroundColor: '#FFFFFF'},
  ...
  warningBox: {flexDirection: 'row', alignItems: 'center', backgroundColor: '#FEFCE8', padding: 12, borderRadius: 8, marginBottom: 16, gap: 8},
  warningText: {fontSize: 12, color: theme.colors.warningDark, flex: 1},
  errorText: {fontSize: 12, color: theme.colors.error, marginTop: 4},
  section: {marginBottom: 24, backgroundColor: '#FFFFFF', padding: 16, borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 16},
  sectionHeader: {flexDirection: 'row', alignItems: 'center', gap: 8},
  sectionTitle: {fontSize: 16, fontWeight: '600', color: '#000000', marginBottom: 8},
  sectionDescription: {fontSize: 14, color: '#64748B', marginBottom: 16},
  exerciseGrid: {flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: 12, marginBottom: 16},
  exerciseCard: {width: '48%', borderWidth: 1, borderColor: '#E2E8F0', backgroundColor: '#FFFFFF', padding: 12, borderRadius: 12, alignItems: 'center', gap: 8},
  selectedCard: {borderColor: theme.colors.primary, backgroundColor: '#EFF6FF'},
  exerciseName: {fontSize: 14, color: '#64748B', textAlign: 'center'},
  selectedText: {color: theme.colors.primary, fontWeight: '600'},
  popularStar: {position: 'absolute', top: 4, right: 4},
  ...
  inputWrapper: {flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 8, paddingHorizontal: 12},
  input: {flex: 1, height: 44, color: '#000000'},
  inputUnit: {fontSize: 14, color: '#64748B', marginLeft: 8},
  attemptsText: {textAlign: 'center', color: '#64748B', marginVertical: 16},
  primaryButton: {backgroundColor: theme.colors.primaryDark, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 16, borderRadius: 8, marginBottom: 50, gap: 8},
  disabledButton: {backgroundColor: '#94a3b8', opacity: 0.7},
  primaryButtonText: {color: '#FFFFFF', fontSize: 16, fontWeight: '600'},
  exerciseInfoItem: {marginBottom: 12},
  exerciseInfoHeader: {flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4},
  exerciseInfoName: {fontWeight: '600', color: theme.colors.primaryDark},
  exerciseInfoDescription: {color: '#64748B', fontSize: 14, marginLeft: 28},
});
